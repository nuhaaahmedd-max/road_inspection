    ...
    if not df_plot.empty:
        ...
        if view_mode == "Points":
            for index, row in df_plot.iterrows():
                ...
                html_content = f'<div style="text-align:center; width:150px;"><b>{row["Object"]}</b><br><img src="data:image/jpeg;base64,{img_b64}" style="width:100%"># ضيفي السطرين دول جوه متغير الـ html اللي في كودك (تحت الصورة مثلاً)
<div style="margin-top:10px; padding:5px; border-top:1px solid #ccc; font-family:monospace;">
    <p style="margin:0; font-size:11px;"><b>Lat (Y):</b> {row['Longitude']:.6f}</p>
    <p style="margin:0; font-size:11px;"><b>Long (X):</b> {row['Latitude']:.6f}</p>
</div>' if img_b64 else "Loading..."
                folium.CircleMarker(
